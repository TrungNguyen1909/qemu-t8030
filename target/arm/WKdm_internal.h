...
#define RECORD_EXACT(queue_posn)  do { EMIT_BYTE(next_tag, EXACT_TAG);  \
                                       EMIT_BYTE(next_qp, (queue_posn));\
                                     } while (0)

#define RECORD_PARTIAL(queue_posn, low_bits_pattern) do { \
   EMIT_BYTE(next_tag, PARTIAL_TAG);                      \
   EMIT_BYTE(next_qp, (queue_posn));                      \
   EMIT_WORD(next_low_bits, (low_bits_pattern))  } while (0)

#define RECORD_MISS(word_pattern) do { EMIT_BYTE(next_tag, MISS_TAG);             \
